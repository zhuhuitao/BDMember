...
public class SuggestionActivity extends BasicActivity {
    ...
    private void commitContent() {
        ...
        loading(true);
        String mSourceId = SpUtils.getString(Contants.Id);
        ...
        Log.i("zdp", "json=" + json);
        mMyOkhttp.post().url(HttpUrl.AddFeedBack).jsonParams(json).tag(this).enqueue(new GsonResponseHandler<Entity>() {
            @Override
    ...
    private void uploadPicture(String path) {
        new AsyncTask<String, Integer, String>() {
            ...
            protected void onPostExecute(String s) {
                ...
                map.put("Base64Str", s);
                mMyOkhttp.post().url(HttpUrl.FeedbackImgLoad).params(map).tag(this).enqueue(new GsonResponseHandler<ResponseEntity<PicturePathEntity>>() {
                    @Override
